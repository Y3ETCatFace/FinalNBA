...
def master_id_endpoint(game_id, endpoint):
    try:
        if endpoint == endpoint_map["playbyplay"]:
            endpoint_class = PlayByPlayV3(
            game_id=game_id,
            end_period=10,
            start_period=1,
            timeout=50
            ...
            endpoint_class = endpoint_map[endpoint](game_id=game_id, timeout=50)
    except (ReadTimeout, ConnectTimeout):
        raise
    except:
        print(f"Error fetching data for game ID {game_id} using endpoint {endpoint}.")
        return None
    return endpoint_class.get_data_frames()[0]
...
def getRandom():
    ...
    for x in range(total_itterations):
        ...
        try:
            ...
            print("failed...waiting 10 seconds for reset api")
            time.sleep(10)
            pass
        time.sleep(1)
...
def creategameIDdata(data_type):
    ...
    if files == game_set:
        ...
        attempt = 0
        request_counter = 1
        print(f"---------------------\nNow buiding {data_type} data for {season}")
        while game_list:
            ...
            try:
                ...
                if df is None:
                    print(f"Endpoint doesn't exist to fetch data for game ID {current_id}. Skipping...")
                    pd.DataFrame().to_csv(data_path, index=False)  # Save an empty DataFrame to maintain consistency
                    print(f"{data_type} Skipped! Becuase {current_id} has no endpoint {calculate_percent_completion(current_game_data,total_game_data)}")
                    continue
