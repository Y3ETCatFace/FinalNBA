...
from tool_config import avg_opp_elo, is_home, days_since, is_back_to_back, games_played
...
def create_avg_opp_elo_col(df, con):
    df['elo'] = pd.NA
...
#--------------------------------------------------------------
    ...
    df.drop(columns=['opponent_id', 'elo', 'matchup', 'season_id'], inplace=True)
...
if __name__ == "__main__":
    ...
    if avg_opp_elo:
        df = con.sql(f"SELECT {grain}, season_id, plus_minus, matchup FROM all_games ORDER BY game_date, game_id").df()
        create_avg_opp_elo_col(df, con)
    if is_home:    
        create_is_home(con)
    if days_since:
